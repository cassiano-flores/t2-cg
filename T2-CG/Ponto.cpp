...
Ponto MontaVetor(Ponto A, Ponto B){
    Ponto V;
    V.x = B.x - A.x;
    V.y = B.y - A.y;
    V.z = B.z - A.z;
...
double ModuloParaAngulo(Ponto v1, Ponto v2)
{
    double v1Raiz = sqrt( pow(v1.x, 2) + pow(v1.y, 2) + pow(v1.z, 2));
    double v2Raiz = sqrt( pow(v2.x, 2) + pow(v2.y, 2) + pow(v2.z, 2));

    return v1Raiz*v2Raiz;
}

double Angulo(Ponto v1, Ponto v2)
{
    double angulo = 0;
    //cosA = (V1 * V2) / (|V1| * |V2|)
    double cossenoA = ProdEscalar(v1,v2) / ModuloParaAngulo(v1,v2);

    angulo = acos(cossenoA)*180/3.1415;
    return angulo;
}
